...
async function insertFighter(name: string, result: string) {
    ...
    console.log(rows)
    if (result === 'win') {
        return db.query(`UPDATE fighters SET wins=$1  WHERE username=$2`, [rows.wins + 1, name]);
    }
    if (result === 'loss') {
        return db.query(`UPDATE fighters SET losses=$1 WHERE username=$2`, [rows.loss + 1, name]);
    }
    if (result === 'draw') {
        return db.query(`UPDATE fighters SET draws=$1 WHERE username=$2`, [rows.draws + 1, name]);
    }
}
